    ...
    for index, row in artists.iterrows():
        if row['start_of_career'] <= year <= row['end_of_career']:
            if row['Gender'] == 1:
                ...
                male += 1
    print(str(female)+"\t"+str(male)+"\n")
    barplot_array.append([year, male, female])


barplot = pd.DataFrame(barplot_array, columns=['year', 'male', 'female'])
barplot = barplot.astype({'year':'string'})
print(barplot.dtypes)
barplot.to_csv("./data/barplot.csv", index=False)
